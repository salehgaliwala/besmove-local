...
class Thrive_Leads_State_Manager extends Thrive_Leads_Request_Handler {
	...
	public function regenerate_css_ids( & $variation ) {
		...
		if ( ! empty( $variation['tcb_fields'] ) && ! empty( $variation['tcb_fields']['globals'] ) && ! empty( $variation['tcb_fields']['globals']['content_css'] ) ) {
			/** replace it in the TCB fields(which are saved in the db) and also in the variation fields(no idea where those are used) */
			$old_custom_css_id                                 = $variation['tcb_fields']['globals']['content_css'];
			$new_custom_css_id                                 = 'tve-u-' . $this->generateRandomString();
			$variation['tcb_fields']['globals']['content_css'] = $new_custom_css_id;
			...
			$variation['tcb_fields']['inline_css']             = str_replace( $old_custom_css_id, $new_custom_css_id, $variation['tcb_fields']['inline_css'] );
