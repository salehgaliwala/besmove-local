...
function thrive_shortcode_tabs_group( $attr, $content ) {
	...
	$container_class = ( $attr['layout'] == "vert" ) ? "vert_tabs" : "horz_tabs";

	$output = '<div class="shortcode_tabs ' . $container_class . '"><div class="scT"><ul class="clearfix scT-tab">' . do_shortcode( $content ) . '</ul>' . $thrive_tabs_divs . "</div></div>";
...
function thrive_shortcode_custom_menu( $attr, $content ) {
	...
	foreach ( $menu_items as $item ) {
		...
		if ( $attr['thumbnails'] == "on" ) {
			...
			$output .= "<div class='pps clearfix'><div class='tim'>";
			if ( $featured_img ) {
			...
			$output .= "<div class='pps clearfix'><div class='txt noImageTab'>";
			$output .= "<a href='" . $item->url . "'>" . get_the_title( $item->object_id ) . "</a><br/>";
			$output .= "</div><div class='clear'></div></div>";
		}
	}
...
function thrive_shortcode_custom_phone( $attr, $content ) {
	...
	$output = "<div class='clear'></div><div class='phone " . $attr['color'] . "'><a href='tel:" . $attr['phone_no'] . "'><div class='phr'><span class='fphr'>";
	...
	$output .= "</div></a></div><div class='clear'></div>";
...
function thrive_shortcode_tab( $attr, $content ) {
	...
	if ( $position == 1 ) {
		...
		if ( $thrive_tabs_layout == "horz" ) {
			...
			$style_attr = " style='display:table-cell;'";
		}
	...
	$output = "<li" . $class_attr . "><a>" . $attr['headline'] . "</a></li>";
	...
	if ( $attr['template'] == 0 ) {
		...
		switch ( $attr['template'] ) {
			case 1:
				...
				break;
			case 2:
				$output = "<div class='clear'></div><div class='" . $out_class . " pattern2 lightSec' style='" . $shadow_string . "'><div class='in '><div class='cck clearfix'><p class=''>" . do_shortcode( $content ) . "</p></div></div></div>";
				break;
			case 3:
...
function thrive_shortcode_pullquote( $attr, $content ) {
	...
	$output = "<blockquote class='" . $container_class . "'><p>" . $content . "</p>" . $cite_html . "<div class='clear'></div></blockquote>";
...
function thrive_shortcode_posts_gallery( $attr, $content ) {
	...
	foreach ( $posts as $p ) {
		...
		if ( $featured_img ) {
			...
			$output .= "<div class='scc-i' style='background-image: url(\"" . get_template_directory_uri() . "/images/default_featured.jpg\");'></div>";
		}
		$output .= "</a>";
		$output .= "<div class='scc-t'>";
		$output .= "<a href='" . get_permalink( $p->ID ) . "'><h3>" . get_the_title( $p->ID ) . "</h3><span class='scc-in'><p>" . $continueReadingLabel . "</p></span></a>";
		$output .= "</div>";
		$output .= "</div>";
	}
	$output .= "</div></div>";
	...
	foreach ( $posts as $p ) {
		if ( $attr['thumbnails'] == "on" ) {
			...
			if ( $featured_img ) {
				...
				$output .= "<a href='" . get_permalink( $p->ID ) . "' style='background-image: url(\"" . get_template_directory_uri() . "/images/default_featured.jpg\")'></a></div>";
			}
			$output .= "<div class='txt'>";
			...
			$output .= "<div class='pps clear'><div class=' txt noImageTab'>";
			$output .= "<a href='" . get_permalink( $p->ID ) . "'>" . get_the_title( $p->ID ) . "</a>";
			if ( $attr['filter'] == "popular" ) {
				$output .= "<span class='thrive_date'>" . get_comments_number( $p->ID ) . " " . _thrive_get_comments_label( get_comments_number( $p->ID ) ) . "</span></div><div class='clear'></div></div>";
			} else {
...
function thrive_shortcode_optin( $attr, $content ) {
	...
	if ( $attr['size'] != "small" && $attr['size'] != "big" ) {
		$attr['size'] = "medium";
	}
...
function thrive_shortcode_fill_counter( $attr, $content ) {
	...
	$output = '
        <div class="fill-counter nsd ' . $attr['color'] . '">
            <div class="fill-counter-inner" style="stroke-dasharray: ' . ( $attr['percentage'] * 6.16 ) . ' 616 ;">
                <svg viewBox="0 0 196 196" class="fill-counter-circle" shape-rendering="optimizeSpeed">
                    <circle r="98" cx="98" cy="98"></circle>
                </svg>
...
function thrive_shortcode_split_button( $attr, $content ) {
	...
	$output = '<div class="split">
                    <a href="' . $attr['left_link'] . '" class="spl small ' . $attr['left_color'] . ' left">
                        <span>' . $attr['left_text'] . '</span>
                    </a>
                    <a href="' . $attr['right_link'] . '" class="spl small ' . $attr['right_color'] . ' right">
...
function _thrive_generate_price_shortcode_output( $attr, $content, $type, $position ) {
	...
	$output .= "<div class='" . $in_div_class . "'>
                    <div class='prb'><div class='prb-i'>
                        <h3>" . $attr['title'] . "</h3></div>";

	$output .= "<p>" . $attr['price'];
	if ( ! empty( $attr['time'] ) ) {
		$output .= "<span>/" . $attr['time'] . "</span>";
	}
	$output .= "</p></div><ul class='check'>";
	...
	$output .= "</ul><div class='prc'>
                <a href='" . $attr['btn_link'] . "' class='btn aligncenter " . $attr['btn_size'] . " " . $attr['btn_color'] . "'>
                        <span>" . $attr['btn_text'] . "</span>
                </a>";
	$output .= "</div></div></div>";
