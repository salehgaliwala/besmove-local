...
if ( isset( $_REQUEST['tve'] ) && $_REQUEST['tve'] == "true" ) {
	$isTvePage = true;
	add_action( 'tcb_hook_editor_head', 'thrive_print_shortcodes' );
	add_action( 'tcb_hook_editor_footer', "thrive_add_quicktags" );
	add_action( 'tcb_hook_template_redirect', 'thrive_enqueue_scripts_for_tve' );
	if ( version_compare( $wp_version, "3.9" ) >= 0 ) {
		add_action( 'tcb_hook_template_redirect', 'thrivev2_add_my_tc_button' );
	} else {
