...
class Thrive_Author_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$thrive_social = array(
			"tw" => get_the_author_meta( 'twitter', $instance['user_profile'] ),
			"fb" => get_the_author_meta( 'facebook', $instance['user_profile'] ),
			"gg" => get_the_author_meta( 'gplus', $instance['user_profile'] )
		);

		$author_url = $instance['user_profile'] ? get_the_author_meta( 'thrive_author_website', $instance['user_profile'] ) : '';
		?>
			...
			<div class="scn">
				<div class="aiv-e">
					<?php if ( $instance['custom_image'] && $instance['custom_image'] != "" ): ?>
						<div class="aiv"
						     style="background-image: url('<?php echo $instance['custom_image'] ?>')"></div>
					<?php elseif ( $author_email ): ?>
						<div class="author-avatar">
							<?php echo get_avatar( $author_email, 95 ); ?>
						</div>
					<?php endif; ?>
				</div>
				<p class="at-t"><?php echo $title; ?></p>
				<?php echo wpautop( $instance['custom_text'] ); ?>
				<?php if ( $author_url ) : ?>
					<a class="mrb" href="<?php echo $author_url; ?>">
						<span><?php _e( "Visit my site", 'thrive' ); ?></span>
					</a>
				<?php elseif ( $instance['user_profile'] ) : ?>
					<a class="mrb" href="<?php echo esc_url( get_author_posts_url( $instance['user_profile'] ) ); ?>">
						<span><?php _e( "Read more", 'thrive' ); ?></span>
					</a>
				<?php endif; ?>
			</div>
		</section>
