 ...
 */
function thrive_get_plusones_shares( $url, $network = 'googleplus' ) {
...
function thrive_get_linkedin_share( $post_link ) {
	$request  = wp_remote_get( "http://www.linkedin.com/countserv/count/share?url=" . $post_link . "&lang=en_US&format=json" );
	$response = json_decode( wp_remote_retrieve_body( $request ) );

	return isset( $response->count ) ? $response->count : 0;
}

function thrive_get_twitter_share( $post_link ) {
	return 0;
	//twitter count api is not supported anymore
	$request  = wp_remote_get( "http://urls.api.twitter.com/1/urls/count.json?url=" . $post_link );
	$response = json_decode( wp_remote_retrieve_body( $request ) );

	return isset( $response->count ) ? $response->count : 0;
}

function thrive_get_facebook_share( $post_link ) {
	return tve_dash_fetch_share_count_facebook( $post_link );
}

function thrive_get_pinterest_share( $post_link ) {
	$request  = wp_remote_get( "http://api.pinterest.com/v1/urls/count.json?url=" . $post_link );
	$request  = preg_replace( '/^receiveCount\((.*)\)$/', "\\1", $request );
	$response = json_decode( wp_remote_retrieve_body( $request ) );
